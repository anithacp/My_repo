...
print_md_me () {
	...
	show "MD Level: "$2
	show "MD Name Format: "$3
	show "MD Name1: "$4
	show "MD Name 2: "$5
	show "MHF Creation Flag: "$6
	show "Sender ID PErmission: "$7
}
...
print_ma_me () {
	...
	show "MD Instance: "$2
	show "MA Name Format: "$3
	show "MA Name1: "$4
	show "MA Name2: "$5
	show "CCM Interval: "$6
	show "Associated VLAN: "$7
	show "MHF Creation: "$8
	show "Sender Id Permssion: "$9
}
...
print_mep_me () {
	...
	show "LTM Priority: "$8
	show "Egress Id: "$9
	show "Eth AIS Ctrl: "$10
	show "Fault Alarm Thresh: "$11
}
...
# $4 - MHF creation
# $5 - MD Name format
# $6 - MD name updation flag (=-1: no update, 1: name 1 updation, 2: name 2 updation)
# $7 - MD name value if $6 flag set to 1 or 2
update_md_me () {
	EntIns=$(printf "00 %.2d" $2)

	if [ $3 -ne -1 ];then
		MDLevel=$(printf "%.2d" $3)
		omci_msg=$(printf "$cmd 10 %.2x $OMCI_MSG_SET_ID $OMCI_DEVICE_ID $MD_ME_CLASS_ID $EntIns 80 00 $MDLevel\n" $tx_id)
	fi
	if [ $4 -ne -1 ];then
		MDNameFormat=$(printf "%.2x" $4)
		omci_msg=$(printf "$cmd 10 %.2x $OMCI_MSG_SET_ID $OMCI_DEVICE_ID $MD_ME_CLASS_ID $EntIns 40 00 $MDNameFormat\n" $tx_id)
	fi
	if [ $5 -ne -1 ];then
		MHFCreation=$(printf "%.2d" $5)
		omci_msg=$(printf "$cmd 10 %.2x $OMCI_MSG_SET_ID $OMCI_DEVICE_ID $MD_ME_CLASS_ID $EntIns 08 00 $MHFCreation\n" $tx_id)
	fi
	if [ $6 -eq 1 ];then
		newMDName1="$7"
		omci_msg=$(printf "$cmd 10 %.2x $OMCI_MSG_SET_ID $OMCI_DEVICE_ID $MD_ME_CLASS_ID $EntIns 20 00 $newMDName1\n" $tx_id)
	fi
	if [ $6 -eq 2 ];then
		newMDName2="$7"
		omci_msg=$(printf "$cmd 10 %.2x $OMCI_MSG_SET_ID $OMCI_DEVICE_ID $MD_ME_CLASS_ID $EntIns 10 00 $newMDName2\n" $tx_id)
	fi
...
execute () {
	case "$test_case_num" in
		1)
			...
			# Get responses for .ag LBM/LTM as well as y1731 dmm, slm msgs
			##############################
		...
		2)
			##############################
			# TC: Create 1 MD. Change MD level with Set message.
			# Result: MD updated correctly
			##############################
			create_md_me $test_case_num 1 1 4 1
			update_md_me $test_case_num 1 5 -1 -1 -1
			;;

		3)
			##############################
			# TC: Create 1 MD with MD Name format as "None" and send set message for MDname1
			# Result: Failure in Set Message
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			update_md_me $test_case_num 1 -1 -1 -1 1 "30 31 32 33 34 00"
			;;
		...
		4)
			...
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 3
			create_ma_me $test_case_num 1 1 $AS_MD_LEVEL 1 $VLAN1 4
			;;
		5)
			##############################
		...
		8)
			...
			# Result: MEP created. Send level 6 LM/SLM/DM messages. No responses.
			# Send level 6 LTM/LBM messages. Get responses.
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
		...
		9)
			...
			# Result: Send LBM with prio=3, get LBR with prio=3
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
		...
		10)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP with LTMprio = 4. No effect on LBR
		...
		11)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP with LTMprio = 7 //255 
			# Result: Send LTM with prio=2, get LTR with prio=7
			# No effect on LBR
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 7
			;;
		...
		13)
			...
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
			update_mep_me $test_case_num 1 4 -1 -1 -1 -1 -1 -1
			;;
		14)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP. 
		...
		15)
			...
			create_ma_me $test_case_num 2 2 $NBN_MD_LEVEL 1 $VLAN2 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
			update_mep_me $test_case_num 1 -1 -1 2 -1 -1 -1 -1
			;;
		...
		17)
			...
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
			update_mep_me $test_case_num 1 -1 -1 -1 10 -1 -1 -1
		...
		20)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP. Send Y.1731 messages.
			# Result: Get Y.1731 responses.
			# Send OMCI SET message to change MEP control bit mask = 10 (no Y.1731).
			# Result: No Y.1731 responses.
			# Send another SET message to change MEP control bit mask = 14
			# Result: Receive responses to Y.1731 messages
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
		...
		21)
			...
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
			update_ma_me $test_case_num 1 2 -1 -1 -1 -1
			;;
		...
		23)
			...
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 2 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
		...
		26)
			##############################
			# TC: Create 1 MD, 1 MA @ AS level. Update MA with MHFCreation = 1 (None)
			# Result: MHF should be deleted (dont get responses for oam msgs LTM/LBM to that MHF)
			# Again send Set msg to update MA with MHFCreation = 2 (Default.
			# Result: MHF should be created (get responses for oam msgs LTM/LBM to that MHF)
			##############################
			create_md_me $test_case_num 1 1 $AS_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $AS_MD_LEVEL 1 $VLAN1 2
			update_ma_me $test_case_num 1 -1 -1 -1 1 -1
			sleep 10
			update_ma_me $test_case_num 1 -1 -1 -1 2 -1
			;;
		...
		28)
			...
			# Result: MHF should be deleted (dont get responses for oam msgs to that MHF)
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
		...
		29)
			##############################
			# TC: Create 1 MD with MD Name format as "None" and send set message for MDname2
			# Result: Failure in Set Message
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			update_md_me $test_case_num 1 -1 -1 -1 2 "30 31 32 33 34 00"
			;;
		30)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP. 
			# Send OMCI SET message to change BPCD Pointer which has TP Type!=1(UNI).
			# Result: MEP created with initial values. MEP updation fails. 
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
		...
		31)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP. 
			# Send OMCI SET message to change MA Pointer which dosent exist.
			# Result: MEP created with initial values. MEP updation fails
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
		...
		32)
			##############################
			# TC: Create 1 MD, 1 MA  and 1 MEP. Update MA with non-existant MD Pointer
			# Result: MA updation fails
			##############################
			create_md_me $test_case_num 1 1 $NBN_MD_LEVEL 1
			create_ma_me $test_case_num 1 1 $NBN_MD_LEVEL 1 $VLAN1 2
			create_mep_me $test_case_num 1 1 1 $VLAN1 0
...
if [ $# -ne 1 ]; then
	...
	echo "Expected Result: All succeeds"
	echo "2: Create 1 MD with wrong MD level."
	echo "Expected Result: MD not created"
	echo "3: Create 1 MD with MD Name format as "None" and send set message for MDname."
	echo "Expected Result: Error in handling set message"
	echo "4. Create 1 MD, 1 MA with MHF setting as "Defer"."
	echo "Expected Result: MA to use same MHF setting as parent MD"
	echo "5. Create 1 MD, 1 MA  and 1 MEP with incorrect BPCD ptr"
	echo "Expected Result: Failure in MEP creation"
	...
	echo "Expected Result: MEP created. No responses to Y.1731 messages"
	echo "9. Create 1 MD, 1 MA  and 1 MEP with LTMprio = 0"
	echo "Expected Result: Send LTM with prio=5, get LTR with prio=5"
	echo "10. Create 1 MD, 1 MA  and 1 MEP with LTMprio = 4"
	echo "Expected Result: Send LTM with prio=5, get LTR with prio=4"
	...
	echo "Expected  Result: MEP created with initial values. MEP reset to changed value in SET"
	...
	echo "Expected Result: No responses to Y.1731 messages"
	...
	echo "Send another SET message to change MEP control bit mask = 14."
	echo "Expected Result: Receive responses to Y.1731 messages"
	echo "21. Create 1 MD, 1 MA  and 1 MEP. Update MA with new MD Pointer."
	echo "Expected Result: MA should be updated correctly"
	...
	echo "Expected Result: MEP should be deleted"
	...
	echo "Expected Result: MEP should be deleted"
	echo "29. Create 1 MD with MD Name format as "None" and send set message for MDname2"
	echo "Expected Result: Failure in Set Message"
	echo "30. Create 1 MD, 1 MA  and 1 MEP. Send OMCI SET message to change BPCD Pointer which has TP Type!=1(UNI)."
	echo "Expected Result: MEP created with initial values. MEP updation fails."
	echo "31. Create 1 MD, 1 MA  and 1 MEP. Send OMCI SET message to change MA Pointer which dosent exist."
	echo "Expected Result: MEP created with initial values. MEP updation fails"
	echo "32. Create 1 MD, 1 MA  and 1 MEP. Update MA with non-existant MD Pointer"
	echo "Expected Result: MA updation fails"
	echo "33. Create 2 MD, 2 MA in NBN and AS levels respectively. MD for AS level has MHF creation = 1 (None) and MA for AS level has MHF creation = 1 (None)"
	echo "Expected Result: No upMHF and dnMHF created @ AS level"
	echo "34. Create 2 MD, 2 MA in NBN and AS levels respectively. MD for AS level has MHF creation = 1 (None) and MA for AS level has MHF creation = 2 (Default)"
	echo "Expected Result: No upMHF and dnMHF created @ AS level"
	echo "35. Create 2 MD, 2 MA in NBN and AS levels respectively. MD for AS level has MHF creation = 2 (Default) and MA for AS level has MHF creation = 1 (None)"
	echo "Expected Result: No upMHF and dnMHF created @ AS level"
	echo "36. Create 2 MD, 2 MA in NBN and AS levels respectively. MD for AS level has MHF creation = 1 (None) and MA for AS level has MHF creation = 4 (Defer - use parent MD MHF flag value)"
	echo "Expected Result: No upMHF and dnMHF created @ AS level"
	echo "37. Create 2 MD, 2 MA in NBN and AS levels respectively. MD for AS level has MHF creation = 2 (Default) and MA for AS level has MHF creation = 4 (Defer - use parent MD MHF flag value). Dont set the uci value for MIP LTR prio. Send LBM msgs with prio 0 to the AS MIP"
	echo "Expected Result: upMHF and dnMHF created @ AS level. AS MIP setup and the LTR has prio 7 (default value)"
	echo "38. Set the uci value for MIP LTR prio to 5. Create 2 MD, 2 MA in NBN and AS levels. MD for AS level has MHF creation = 2 (Default) and MA for AS level has MHF creation = 2 (Default) Send LBM msgs with prio 0 to the AS MIP"
	echo "Expected Result: AS MIP setup and the LTR has prio 5"
	echo "#####################################################################################"
	exit 1
fi 
